...
import { permissions as permissionsArrary } from "../../../objects/apiResponses";
...
const Create = (props) => {
  ...
  const createRoleButtonInfo = {
    ...
    onClick: function (ev) {
      ...
      if (!nameError && name != "") {
        ...
        props
          .authAxios({ ...props.apiEndPoints.roles.create, data })
          .then((res) => {
  ...
  useEffect(() => {
    ...
    const successResponse = permissionsArrary;
    log("successResponse", successResponse);
    ...
    <Box>
      ...
      <Grid container direction="column">
        <Grid item mb={3}>
          <Heading
            title="Create Role"
            button={{ ...createRoleButtonInfo, disabled: isProcessingRequest }}
          />
          <Box
            sx={{
              width: "100%",
...
export default withAxios(Create);
