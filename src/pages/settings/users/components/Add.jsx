...
import { roles as rolesArray } from "../../../../objects/apiResponses";
...
const Add = (props) => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      email: "",
      role_id: "",
      account_uuid: userInfo.user.account.uuid,
      prevent_email: false,
      system_refer: userInfo.user.system_refer,
    },
    validateOnBlur: false,
    onSubmit: (values, formikBag) => {
      props
        .authAxios({ ...props.apiEndPoints.users.addUser, data: values })
        .then((res) => {
          const successResponse = res.data;
          log("successResponse", successResponse);

          props.setParentState.setSnackbarInfo({
  ...
  useEffect(() => {
    ...
    const successResponse = rolesArray;
    log(successResponse);
