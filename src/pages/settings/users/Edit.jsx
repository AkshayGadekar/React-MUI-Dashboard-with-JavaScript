...
const Edit = (props) => {
  ...
  const loggedInUserEmail = userInfo.user.email;
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, formikBag) => {
      props
        .authAxios({
          ...props._(props.apiEndPoints.users.editUser, { id: param.id }),
          data: values,
        })
        .then((res) => {
        ...
        .catch((error) => {
          props.processAxiosError(error, props, formik);
        })
        .finally(() => {
  ...
  const editRoleBtn = {
    value: "Edit Role",
    component: Link,
    to: replaceDynamicParamInHref(editRoleActionHref, [
      Object.keys(userData).length ? userData.role.uuid : "",
    ]),
  };
  ...
  useEffect(() => {
    ...
    */
    props.setShowSnackBar(false);
    const successResponse = usersListing.users[0];
    log(successResponse);
    setUserData(successResponse);
  ...
  const userPermissionsIds = useMemo(() => {
    return userPermissions.map((userPermission) => userPermission.id);
        ...
        <>
          ...
          <Grid container>
            <Grid
              ...
              sx={{
                marginBottom: "1.5rem",
                [theme.breakpoints.up("md")]: { paddingRight: "1rem" },
              }}
            >
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={12} md={6}>
                      <TextField
                        ...
                        helperText={
                          FirstNameTouchedError && formik.errors.first_name
                        }
                      />
                    </Grid>
                    <Grid item xs={12} md={6}>
                      <TextField
                        ...
                        helperText={
                          LastNameTouchedError && formik.errors.last_name
                        }
                      />
                    </Grid>
                    <Grid item xs={12} md={6}>
                    ...
                    <Grid item xs={12} md={6}>
                      <FormControl fullWidth error={RoleTouchedError}>
                        ...
                        <FormHelperText error>
                          {RoleTouchedError && formik.errors.role_id}
                        </FormHelperText>
                      </FormControl>
                    </Grid>
              ...
              >
                <Grid container direction="column" spacing={1}>
                  <ShowPermissions
                    ...
                    selectedPermissionsIds={userPermissionsIds}
                    disabledChecks
                  />
                </Grid>
              </Box>
